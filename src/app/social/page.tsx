'use client'
import { useEffect } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { fetchSocialPosts } from '@/store/slices/contentSlice'
import { RootState, AppDispatch } from '@/store'
import ContentCard from '@/components/ContentCard'

export default function SocialPage() {
  const dispatch = useDispatch<AppDispatch>()
  const { items, loading } = useSelector((state: RootState) => state.content)

  // Fetch social posts if not already loaded
  useEffect(() => {
    if (!items.some(item => item.type === 'social')) {
      dispatch(fetchSocialPosts())
    }
  }, [dispatch, items])

  const socialItems = items.filter(item => item.type === 'social')

  if (loading && !socialItems.length) return <div>Loading...</div>
  if (!socialItems.length) return <div className="text-center py-12">No social posts available</div>

  return (
    <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {socialItems.map((item, index) => (
        <ContentCard key={item.id} item={item} index={index} />
      ))}
    </div>
  )
}
